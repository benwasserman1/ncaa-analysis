...
# wins per team
tourney_wins_higher_team <- tourney_detailed_result %>% group_by(higher_team) %>% 
  summarize(num_wins = n()) %>%
  arrange(desc(num_wins))
tourney_wins_lower_team <- tourney_detailed_result %>% group_by(lower_team) %>% 
  summarize(num_wins = n()) %>%
  arrange(desc(num_wins))
tourney_wins_per_team <- left_join(x = tourney_wins_per_team,
                                     y = teams, 
                                     by = "higher_team")
tourney_wins_per_team %<>% select(higher_team, TeamName, num_wins)
