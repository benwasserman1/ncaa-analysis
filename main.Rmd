...
# renaming
tourney_detailed_result %<>% rename(WTeamName = TeamName.x,
...
glm_fit <- glm(lower_team_won ~ WTeamSeed + LTeamSeed + WTeamConfFactor,
               data = tourney_selected_vars,
               family = binomial)
summary(glm_fit)

names(tourney_selected_vars)

# not super explainable for the data
mod1 <- lm(scoreDiff ~ WTeamConf + LTeamConf + 
             WTeamSeed + LTeamSeed + Round, data = tourney_selected_vars)
...
tourney_semi_views_ratings <- tourney_sem_views_ratings %>% mutate(
...
# NOW WE CAN RUN REGRESSION ON RATINGS AND VIEWERSHIP
