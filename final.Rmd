...
tourney_d_2017 <- subset(tourney_d_result, Season.x == 2017)
tourney_d_2017_train <- subset(tourney_d_result, as.numeric(Season.x) < 2017)
tourney_d_2016 <- subset(tourney_d_result, Season.x == 2016)
tourney_d_2016_train <- subset(tourney_d_result, as.numeric(Season.x) < 2016)
...
glm_fit_2017 <- glm(higher_team_won ~ SeedDifference + AvgSeedDiff + HigherTeamConfFactor + 
                 LowerTeamConfFactor,
               data = tourney_d_2017_train,
               family = binomial)

glm_fit_2016 <- glm(higher_team_won ~ SeedDifference + AvgSeedDiff + HigherTeamConfFactor + 
                 LowerTeamConfFactor,
               data = tourney_d_2016_train,
               family = binomial)
...
preds_2016_test_DF <- data.frame(
  scores_logit = predict(glm_fit_2016, type="response", newdata = tourney_d_2016),
  tourney_d_2016
)

preds_2017_test_DF <- data.frame(
  scores_logit = predict(glm_fit_2017, type="response", newdata = tourney_d_2017),
  tourney_d_2017
)
...
preds_2017_test_DF <- preds_2017_test_DF %>% mutate(
  class_pred05 = ifelse(scores_logit > .5, "Yes", "No")
)

preds_2016_test_DF <- preds_2016_test_DF %>% mutate(
  class_pred05 = ifelse(scores_logit > .5, "Yes", "No")
)

table(preds_2016_test_DF$class_pred05, preds_2016_test_DF$higher_team_won)
table(preds_2017_test_DF$class_pred05, preds_2017_test_DF$higher_team_won)
...
```{r}
dim(tourney_d_2016)
dim(tourney_d_2016_train)

glm_fit1 <- glm(higher_team_won ~ SeedDifference + AvgSeedDiff + HigherTeamConfFactor + 
                 LowerTeamConfFactor + DFTMReg + DStlReg + DFGAReg + DFGA3Reg + DFGM3Reg + DFTAReg + DORReg + DDRReg
              + DORReg + DTOReg + DAstReg + DBlkReg,
               data = tourney_d_2017_train,
               family = binomial)

glm_fit2 <- glm(higher_team_won ~ SeedDifference + AvgSeedDiff + HigherTeamConfFactor + 
                 LowerTeamConfFactor + DFTMReg + DStlReg + DFGAReg + DFGA3Reg + DFGM3Reg + DFTAReg + DORReg + DDRReg
              + DORReg + DTOReg + DAstReg + DBlkReg,
               data = tourney_d_2016_train,
               family = binomial)
...
preds_2016_DF <- data.frame(
  scores_logit = predict(glm_fit2, type = "response", newdata = tourney_d_2016),
  tourney_d_2016
)

preds_2017_DF <- data.frame(
  scores_logit = predict(glm_fit1, type = "response", newdata = tourney_d_2017),
  tourney_d_2017
)

preds_test_2016 <- preds_2016_DF %>% mutate(
  class_pred05 = ifelse(scores_logit > .35, "Yes", "No")
)
...
table(preds_test_2016$class_pred05, preds_test_2016$higher_team_won)
table(preds_test_2017$class_pred05, preds_test_2017$higher_team_won)

(roc_test <- ggplot(preds_test_2017, aes(m = scores_logit,
                                      d = higher_team_won)) + 
...
lasso_preds <- predict(lasso_fit, type="response", newdata = tourney_d_test, s = lasso_fit$lambda.min)
...
rf_fit2 <- randomForest(as.factor(higher_team_won) ~ SeedDifference + AvgSeedDiff + HigherTeamConfFactor + 
                 LowerTeamConfFactor + DFTMReg + DStlReg + DFGAReg + DFGA3Reg + DFGM3Reg + DFTAReg + DORReg + DDRReg
              + DORReg + DTOReg + DAstReg + DBlkReg,
          data = tourney_d_train,
          type = classification, 
...
# explain_forest(rf_fit2, interactions = TRUE, data = tourney_d_train)

preds_test_DF <- data.frame(
  scores_logit = predict(rf_fit, type = "prob", newdata = tourney_d_2017),
  tourney_d_2017
)

preds_test_2017 <- data.frame(
  scores_logit = predict(rf_fit2, type = "prob", newdata = tourney_d_2017),
  tourney_d_2017
)

View(preds_test_DF)

preds_test_2017 <- preds_test_2017 %>% mutate(
...
lin_mod3 <- lm(Viewers ~ relevel(WTeamConfFactor, ref="Other") + IsUpset + scoreDiff,
               data = tourney_views_ratings)
...
Although there isn't much data, we have performed leave one out cross validation to make sure that our data is training on as much data as possible. We also were able to confirm that the data is homoschedastic so it does appear to fit a linear pattern. That said, we can't extrapolate about these results too much considering the limited quantity of data in comparison to the number of attributes we are analyzing.
